...
export default function ProgramSelectionForm({ formData, handleChange, nextStep, prevStep, errors, mockData }) {
    ...
    useEffect(() => {
        if (formData.university) {
            setAvailableFields(mockData.fields[formData.university] || [])
        } else {
            setAvailableFields([])
        }
    }, [formData.university, mockData.fields])

    const handleSubStepNext = () => {
        if (subStep === 1 && !formData.country) {
            return
        }

        if (subStep === 2 && !formData.university) {
            return
        }

        if (subStep === 3 && !formData.field) {
            return
        }
                ...
                return (
                    <div className="space-y-4">
                        ...
                        <p className="text-sm text-gray-600">
                            Choisissez l'université où vous souhaitez étudier en {formData.country}
                        </p>
            ...
            case 3:
                return (
                    <div className="space-y-4">
                        ...
                        <p className="text-sm text-gray-600">
                            Choisissez la filière que vous souhaitez étudier à {formData.university}
                        </p>
